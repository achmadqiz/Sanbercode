...
describe("OrangeHRM | Login Feature - POM Tests", () => {
  ...
  describe("Positive Cases", () => {
    it("[AUTH-LOGIN-001] Valid credentials redirect to dashboard", () => {
      loginPage.login(data.credentials.admin.username, data.credentials.admin.password);
    ...
    it("[AUTH-LOGIN-002] Lowercase username is case-insensitive", () => {
      loginPage.login(data.credentials.lowercase.username, data.credentials.lowercase.password);
      cy.url().should("include", data.urls.dashboard);
      loginPage.getDashboardHeader().should("be.visible");
    });
  ...
  describe("Negative Cases", () => {
    context("Invalid Credentials", () => {
      ...
      it('[AUTH-LOGIN-007] Username >100 chars shows "Invalid credentials"', () => {
        loginPage.login(data.payloads.long100, data.credentials.admin.password);
        loginPage.getErrorMessage().should("contain.text", "Invalid credentials");
      });

      it('[AUTH-LOGIN-008] Password >100 chars shows "Invalid credentials"', () => {
        loginPage.login(data.credentials.admin.username, data.payloads.long100);
        loginPage.getErrorMessage().should("contain.text", "Invalid credentials");
      });

      it('[AUTH-LOGIN-009] Special chars in username shows "Invalid credentials"', () => {
        loginPage.login("Admin" + data.payloads.specialChars, data.credentials.admin.password);
        loginPage.getErrorMessage().should("contain.text", "Invalid credentials");
      });

      it('[AUTH-LOGIN-010] Special chars in password shows "Invalid credentials"', () => {
        loginPage.login(data.credentials.admin.username, data.payloads.specialChars);
        loginPage.getErrorMessage().should("contain.text", "Invalid credentials");
      });

      it('[AUTH-LOGIN-011] Numeric-only username shows "Invalid credentials"', () => {
        loginPage.login(data.payloads.numericOnly, data.credentials.admin.password);
        loginPage.getErrorMessage().should("contain.text", "Invalid credentials");
      });

      it('[AUTH-LOGIN-012] Numeric-only password shows "Invalid credentials"', () => {
        loginPage.login(data.credentials.admin.username, data.payloads.numericOnly);
        loginPage.getErrorMessage().should("contain.text", "Invalid credentials");
      });
    });
    ...
    context("Required Field Validation", () => {
      it('[AUTH-LOGIN-013] Empty password shows "Required"', () => {
        ...
        loginPage.getRequiredMessages().first().should("contain.text", "Required");
      });
